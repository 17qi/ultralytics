...
def _get_weight_size_mib(weights_path: Path) -> float:
    return weights_path.stat().st_size / (1024 * 1024)


def _get_model_yaml(model: YOLO) -> str:
    yolo_model = model.model
    yaml_dict = getattr(yolo_model, "yaml", None)
    ckpt_train_args = (getattr(model, "ckpt", {}) or {}).get("train_args", {}) or {}

    candidates = [
        getattr(yolo_model, "yaml_file", ""),
        yaml_dict.get("yaml_file", "") if isinstance(yaml_dict, dict) else "",
        ckpt_train_args.get("model", ""),
    ]
...
def main() -> None:
    ...
    params_m = _get_weight_size_mib(weights_path)
    try:
    ...
    summary_path = REPO_ROOT / "tools" / "model_performance_records.csv"
